...
sheet_url = "https://docs.google.com/spreadsheets/d/1Vf6jNCzb4e3FZLgU6ZgY-uKXLEpzBeVh2wVB0oBzh_Y/gviz/tq?tqx=out:csv&sheet=tw_links&tq=select+A"
timestamp = (str(datetime.now()).replace(":", "-"))[:-7]

# ---------------------------------------------------------------------------------------------- FUNKCE


def download_data_from_column(sheet_url_f):
    data = pd.read_csv(sheet_url_f, header=None)
    pic_weblinks = data.iloc[:, 0].tolist()
    return pic_weblinks
...
img_links = download_data_from_column(sheet_url)
